...
def return_data_input(data123):
    x1, y1, z1 = data123[:, 0], data123[:, 1], data123[:, 2]
    #x2, y2, z2 = data_2[:, 0], data_2[:, 1], data_2[:, 2]
    x2=x1**2
    y2=y1**2
    xy=x1*y1
    on=[1 for i in range(data123.shape[0])] # Bias
    return np.column_stack((on,x1,y1,x2,y2,xy))#,x1y1,x2y2

data_train=return_data_input(data)
...
class CustomDense(Layer):
  ...
  def call(self, inputs):  
         ...
         return G_2(tf.Tensor(inputs).numpy(),func1,func2)
...
data_test=return_data_input(new_data1)
plot_data=model.predict(data_test)
print(plot_data)

'''
Using polynomial regression to plot data
'''
...
# make predictions
# Generate New data
data_= generate_intersection_sample(N+200, func1,func2)
data_train=return_data_input(data_)
y_pred = regressor.predict(data_train)



with col2:  
    trace1 = go.Scatter3d(x=x, y=y, z=z,mode='markers', name="Steepest Gradient Descent", marker=dict(size=5,sizemode='diameter'))
    x2, y2, z2 = data2[:, 0], data2[:, 1], data2[:, 2]
    trace2 = go.Scatter3d(x=x, y=y, z=plot_data[:,0],mode='markers', name="From Neural Network", marker=dict(size=5,sizemode='diameter'))
    trace3 = go.Scatter3d(x=data_[:, 0], y=data_[:, 1], z=y_pred,mode='markers', name="From Predicted equation", marker=dict(size=5,sizemode='diameter'))

    fig = make_subplots()
    fig.add_trace(trace1)
    fig.add_trace(trace2)
    fig.add_trace(trace3)
    fig.update_layout(margin=dict(l=0, r=0, b=0, t=0), height=700, width=1000)
    st.plotly_chart(fig, use_container_width=True, height=700, width=1000)
...
st.write('### Equation of the curve as found using multivariable neural network')
