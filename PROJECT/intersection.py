...
from sympy.tensor.array import derive_by_array  # Helps to find gradient of entire equation
import matplotlib.pyplot as plt
import tensorflow as tf

# Define the variables used in the equation. Here we are considering curves which are  
x1 = Symbol('x')
x2 = Symbol('y')
x3 = Symbol('z')
X = Matrix([x1, x2, x3]) # Matrix to store all the variables

# Function 1 equations to be selected
def Function_1(x1, x2, x3, f1_type):
    x = np.array([x1, x2, x3])
    if f1_type == "DoubleCone_1":
        return x3**2 - x1**2 - x2**2
    elif f1_type == "Cylinder_1":
        return 1 - x3**2 - x2**2
    elif f1_type == "Sphere_1":
        a = np.array([0.61, 0.56, 0.46])
        R = 0.5    
        x_a = x-a
        x_a = np.sum((x_a)**2)
        return x_a - R**2

# Function 2 equations to be selected.
def F2(x1, x2, x3, f2_type):
    x = np.array([x1, x2, x3])
    if f2_type == "DoubleCone_2":
        return (x2-0.90)**2 - x1**2 - x3**2
    elif f2_type == "Cylinder_2":
        return 1 - x1**2 - x2**2
    elif f2_type == "Sphere_2":
        a = np.array([0.90, 0.35, 0.57])
        R = 0.5    
        x_a = x-a  # x is a numpy array to store
        x_a = np.sum((x_a)**2)
        return x_a - R**2
...
def F1_curve(l1, f1_type):
    return (Function_1(l1[0],l1[1],l1[2], f1_type))**2

def F2_curve(l1, f2_type):
    return (F2(l1[0],l1[1],l1[2],f2_type))**2

# Graph of intersection is G(x) = (F1(x))^2 + (F2(x))^2. This should be equivalent to zero
def G(l1, f1_type, f2_type):
    return (Function_1(l1[0],l1[1],l1[2],f1_type))**2 + (F2(l1[0],l1[1],l1[2],f2_type))**2

def G_2(l1, f1_type, f2_type):
    z=tf.stop_gradient(l1).numpy()
    return (Function_1(l1[:,0],l1[:,1],l1[:,2],f1_type))**2 + (F2(l1[:,0],l1[:,1],l1[:,2],f2_type))**2
...
# To speed up the process of finding intersection I have used Steepest gradient descent here.

def generate_intersection_sample(N, f1_type, f2_type):
    y = G(X,f1_type, f2_type)

    first_derivative = derive_by_array(y, (x1, x2, x3)) # Gradient
    hessian = derive_by_array(first_derivative, (x1, x2, x3)) # Double derivative 
